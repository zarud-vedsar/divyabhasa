...
if (!$res) {
    if (isset($id) && intval($id)) {
        $update_query = "UPDATE tbl_users SET profile='$profile',   
                                                name='$name',  
                                                phone=$phone, 
                                                state=$state, 
                                                city='$city', 
        ...
        $check_seller = runFatch("SELECT username FROM tbl_users WHERE username = '{$username}' ");
        if ($check_seller) {
            $res['status'] = 101;
            $res['msg'] = 'Seller Already Exist';
        } else {
            $insert_query = "INSERT INTO tbl_users (profile, 
                                                    name, 
                                                    phone, 
                                                    username, 
                                                    state, 
